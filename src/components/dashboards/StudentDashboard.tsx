...
import { MapPin, Clock, Navigation, LogOut, Home, Bell, Map } from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const myBusLocation = locations.find(loc => loc.busId === user?.busId);
    if (myBusLocation) {
      setBusLocation({
      ...
      <div className="bg-slate-800 border-b border-slate-700 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">MY BUS TRACKER</h1>
            ...
            <div className="text-center space-y-3">
              {eta !== null ? (
                <>
                  ...
                  <h2 className="text-2xl font-bold">ETA: {eta} MINUTES</h2>
                  <p className="text-slate-400">Your bus is on the way!</p>
                  <Badge className={eta <= 10 ? "bg-red-500" : eta <= 20 ? "bg-yellow-500" : "bg-green-500"}>
                    {eta <= 10 ? "ARRIVING SOON" : eta <= 20 ? "GETTING CLOSE" : "ON TIME"}
                  </Badge>
                </>
              ) : (
                <>
                  <MapPin className="w-12 h-12 text-slate-400 mx-auto" />
                  <h2 className="text-xl font-bold text-slate-400">BUS NOT ACTIVE</h2>
                  <p className="text-slate-400">Waiting for driver to start route</p>
                </>
              )}
            </div>
          </CardContent>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 text-center">
              ...
              <div className="text-lg font-bold">{busLocation ? 'LIVE' : 'OFFLINE'}</div>
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardContent className="p-4 text-center">
              ...
              <div className="text-lg font-bold">{user?.busId}</div>
